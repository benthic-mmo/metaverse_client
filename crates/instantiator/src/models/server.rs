...
impl Handler<StdoutMessage> for SimServer {
    ...
    fn handle(&mut self, msg: StdoutMessage, ctx: &mut Self::Context) -> Self::Result {
        info!("[{}]: {}", msg.component.to_string(), msg.log_content.to_string());
        match msg.component{
            ServerComponents::Console(_) => {
                if !(self.state == ServerState::Started) && msg.log_content.contains("Currently selected region is"){
...
impl Handler<StartServer> for SimServer {
    ...
    fn handle(&mut self, msg: StartServer, ctx: &mut Self::Context) -> Self::Result {
