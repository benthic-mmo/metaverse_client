...
use metaverse_messages::{chat_from_viewer::{ChatFromViewer, ClientChatType}, packet::Packet};
...
pub fn chat_screen(mut contexts: EguiContexts, mut chat_message: ResMut<ChatMessage>, session_data: Res<SessionData>, sockets: Res<Sockets>, chat_messages: Res<ChatMessages>) {
        ...
        ui.vertical(|ui| {
            egui::ScrollArea::vertical().auto_shrink([false, true]).show(ui, |ui| {
                for (i, message) in chat_messages.messages.iter().enumerate() {
                    ui.push_id(i, |ui| {
                        ui.label(format!("{}: {}", message.user.clone(), message.message.clone())); // Display the message
                    });
                }
            });
        });
